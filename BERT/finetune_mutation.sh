...
export TASK="mutation"

mkdir -p "$RUN_SAVE_PATH"
mkdir -p "$RUN_SAVE_PATH/finetuned/$TASK"

LOGFILE="$RUN_SAVE_PATH/logs/finetune_mutation.log"
: > "$LOGFILE"
exec &> >(tee -a "$LOGFILE")

echo "Starting SFT for task ${TASK}..."

python -m bird.finetune.finetune_bert \
  --task "$TASK" \
  --data_file "$DATA_DIR/dna_${NUM_SEQ}_seq.jsonl" \
  --pretrained_dir "$RUN_SAVE_PATH/checkpoints/bert_k3_overlap/best" \
